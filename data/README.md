...
画面の隅にいる雷神・もりけんが、**押されたときに言う単発のひとこと**です。

**かけあい（会話）はここに置きません。** ふたりのやりとりは「もりけんと雷神の話」
（`data/conversations.js`）が担当します。固定マスコットは愛嬌担当、
Dialogueセクションは読ませる会話、と役割を分けてあります。
...
- `raizin` と `moriken` は**完全に独立**しています。片方に足しても、もう片方には出ません
- **1つで意味が通る文にしてください。**
  「今日は何を作る？」のように返事を待つ形は、相手が答えないので宙に浮きます。
  疑問符で終わる文は自動検査で弾かれます
- `\n` で改行できます。ふきだしは幅が狭いので2行くらいが目安
- クリックされるたびに、書いた順に1つずつ出ます
- 同じ文を二度書くと自動検査で弾かれます
...
- **初回訪問のときだけ**、読み込み2.6秒後に雷神が一度だけ喋ります。
  2回目以降は出ません（`localStorage` の `ls-mascot-greeted` で記録）。
  動きを抑える設定（`prefers-reduced-motion`）のときは出しません
- キーボードでも操作できます（Tabで移動、Enter / Space で表示）
