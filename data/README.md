...
- **自動では出ません**（2026-08-19に廃止）。押されたときだけ喋ります
- 押すたびに、自分のひとこと → かけあい の順に1つずつ出ます
- 表示は7秒。もう一度押すか、7秒経つと消えます
- **初回訪問のときだけ**、読み込み2.6秒後に雷神が一度だけ喋ります。
  2回目以降は出ません（`localStorage` の `ls-mascot-greeted`）。
  動きを抑える設定（`prefers-reduced-motion`）のときは出しません
- キーボードでも操作できます（Tabで移動、Enter / Space）
